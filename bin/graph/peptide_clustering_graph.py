...
@click.option("output_dir", "--output", type=click.Path(exists=True, file_okay=False, dir_okay=True), default=None, help="The output directory to save the graph.")
def main(graph_file: click.Path, peptide_file: click.Path, output_dir: click.Path):
    ...
    for idx, cc in enumerate(cc_list):
        ...
        if is_only_one(cc_peptides):
            peptide_of_cc = get_only_one_peptide(cc_peptides, input_peptides)
            peptide_of_cc.id = f"{peptide_of_cc.id}-cc-{idx}"
            cc_with_only_one_peptide_list.append(peptide_of_cc)
            # maybe we should already store this peptide as cc's epitope in the graph     
            ...
            _write_output_file_(peptides_on_cc_list, f"{output_path}/cc-{idx}.fasta") 
    ...
    # print(cc_with_only_one_peptide_list)
    # pickle.dump(graph, open(f"{output_file_name}-graph-cc-id.pickle", 'wb'))
        
def get_only_one_peptide(peptides, input_peptides):
    peptide_id = next(iter(peptides))
    return input_peptides[peptide_id]

def get_peptides(peptides_ids, idx, input_peptides):
    peptides_on_cc = []
    for pep_id in peptides_ids:
        peptide = input_peptides[pep_id]
        peptide.id = f"{peptide.id}-cc-{idx}"
