...
class SegmentAnnotation:
    def __init__(self, scaffold, start, end, reads_count, min_mapq, max_mapq, avg_mapq, median_mapq, scaffold2, source, feature, start2, end2, score, strand, phase, attributes):
    ...
    def __str__(self):
        return f"{self.scaffold}\t{self.start}\t{self.end}\t{self.reads_count}\t{self.min_mapq}\t{self.max_mapq}\t{self.avg_mapq}\t{self.median_mapq}\t{self.scaffold2}\t{self.source}\t{self.feature}\t{self.start2}\t{self.end2}\t{self.score}\t{self.strand}\t{self.phase}\t{self.attributes}"
